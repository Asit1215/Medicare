...
const AppointmentsPage = () => {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        setAppointments(items); //fetch all the details presents on the DB.
      } catch (err) {
  ...
  //compute available specialites from fetched appointments.
  ...
  //sort filtered by datetime in decending order.
  ...
  async function adminCancelAppointment(id) {
    ...
    //dont all cancel or complete to be overdone.
        ...
        {loading ? (
          ...
          <main className={pageStyles.gridContainer}>
            {displayed.map((a, idx) => {
              ...
              const isCancelled = statusLower === "canceled" || "cancelled";
              const isCompleted = statusLower === "completed";
