import Appoinment from "../models/Appointmets.js";
...
export const getAppointments = async (req, res) => {
  try {
    ...
    const items = await Appoinment.find(filter)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit)
    ...
    const total = await Appoinment.countDocuments(filter);
    return res.json({
      success: true,
      appointments: items,
...
export const createAppointment = async (req, res) => {
  try {
    ...
    const existingBooking = await Appoinment.findOne({
      doctorId,
    ...
    // Free appointment
    if (numericFee === 0) {
      const created = await Appoinment.create({
        ...base,
        status: "Confirmed",
...
export const confirmPayment = async (req, res) => {
  try {
    ...
    let appt = await Appoinment.findOneAndUpdate(
    ...
    if (!appt) {
      ...
      if (meta.doctorId && meta.mobile && meta.patientName) {
        appt = await Appoinment.findOneAndUpdate(
          {
...
export const updateAppoinment = async (req, res) => {
  try {
    const { id } = req.params;
    const body = req.body || {};
    const appt = await Appoinment.findById(id);
    ...
    const updated = await Appoinment.findByIdAndUpdate(id, update, {
      new: true,
      runValidators: true,
    })
    ...
    console.error("UpdateAppoinment error: ", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};
...
export const getStats = async (req, res) => {
  try {
    const total = await Appoinment.countDocuments();
    ...
    const recent = await Appoinment.countDocuments({
      createdAt: { $gte: savenDaysAgo },
    });

    return res.json({
      success: true,
      stats: { total, revenue, recent },
    });
  } catch (err) {
...
export const getAppointmentsByDoctor = async (req, res) => {
    ...
    const items = await Appoinment.find(filter).sort({date: 1, time: 1}).skip(skip).limit(limit).populate("doctorId", "name specialization owner imageUrl image").lean();
