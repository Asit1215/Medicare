...
// HELPER FUNTION
// This function will convert time 12hours to number of minutes since midnight;
// and return AM : PM
...
// This funtion will remove duplicate slot and return the slot filter by time
...
// This funtion accept the obj or JSON string
...
export async function deleteDoctor(req, res) {
  try {
    ...
    return res.json({ success: true, message: "Doctor deleted Succesfully" });
  } catch (error) {
...
export async function doctorLogin(req, res) {
    try {
        ...
        if(!secret) return res.status(500).json({
            success: false,
            message: "Server Misconfigurd"
        });

        const token = jwt.sign({
        id: doc._id.toString(),
        email: doc.email,
        role: "doctor" }, secret, { expiresIn: "7d" });
